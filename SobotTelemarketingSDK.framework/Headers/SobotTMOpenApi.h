...
//
//  Created by zhangxy on 2023/12/15.
//
...
//webstom的消息订阅
typedef BOOL (^SobotTMApiScribeMessage)(NSString * _Nullable common,NSDictionary *header,NSString *messageId,NSDictionary *content);

@interface SobotTMOpenApi : NSObject

@property(nonatomic,strong) SobotTMApiScribeMessage callScribeMessageHandler;
...
+(void)initWithConfig:(SobotCallCacheEntity *) config result:(SobotTMResultBlock) resultBlock;

//第二步
///  登录，更多登录方式，参见[SobotCallCache doLogin:...]
/// @param account 账号，必须
/// @param loginPwd 密码
/// @param token 公司token（如果不传递密码，使用单点登录获取的token）
...
///
///
+(void)queryWebmsgUrls:(SobotTMResultBlock) resultBlock;
...
+(void)updateExtLangCode:(NSString *) ext langCode:(NSString *) langCode result:(SobotTMResultBlock)resultBlock;
...
/// 重置离线
/// @param resultBlock 结果
+(void)logResetOut:(SobotTMResultBlock)resultBlock;

/// 置忙
/// @param reasonCode 置忙原因  、 11：小休；12：小休（培训）；13：小休（会议）；14：小休（用餐）；15：小休（活动）；16：小休（自定义1）；17：小休（自定义2）；18：小休（自定义3）
/// @param resultBlock 结果回调
+(void)noReady:(int )reasonCode resultBlock:(SobotTMResultBlock)resultBlock;

/// 置闲
/// @param resultBlock 结果回调
+(void)ready:(SobotTMResultBlock)resultBlock;

/// 座席重置离线
/// @param resultBlock 结果回调
+(void)resetLogout:(SobotTMResultBlock)resultBlock;
...
///  接听
/// @param resultBlock 结果回调
+(void)answer:(SobotTMResultBlock)resultBlock;

///   挂机
/// @param resultBlock 结果回调
+(void)hangup:(SobotTMResultBlock)resultBlock;

///   保持
/// @param resultBlock 结果回调
+(void)holdCall:(SobotTMResultBlock)resultBlock;

///   取消保持
/// @param resultBlock 结果回调
+(void)retrieveCall:(SobotTMResultBlock)resultBlock;

///   静音
/// @param resultBlock 结果回调
+(void)muteCall:(SobotTMResultBlock)resultBlock;

///   取消静音
/// @param resultBlock 结果回调
+(void)unmute:(SobotTMResultBlock)resultBlock;

/// 查询播放类型
+(NSString *)searchPlayCategory;

///设置播放类型
+(void)changedPlayCategory:(SobotTMResultBlock) resultBlock;

///  发送满意度
/// @param resultBlock 结果回调
+(void)sendSatisfy:(SobotTMResultBlock)resultBlock;

///  延长整理时长
/// @param delayTime  延长时间 (30-900秒)
/// @param resultBlock 结果回调
+(void)delayACW:(int )delayTime ResultBlock:(SobotTMResultBlock)resultBlock;

///  结束整理
/// @param resultBlock 结果回调
+(void)completeACW:(SobotTMResultBlock)resultBlock;

/// 结束整理并置忙
/// @param resultBlock 结果回调
+(void)completeACWToBusy:(SobotTMResultBlock)resultBlock;
...
/// 查询座席的外呼路由规则
/// @param resultBlock 结果回调
+(void)queryRoutes:(SobotTMResultBlock)resultBlock;
